...
def search_recos(film):
    ...
    link = "dataframe_recommandation.parquet"
    ...
    if film:
        ...
        if film in df['originalTitle'].values:
            ...
            recommendations = df.loc[df['originalTitle'] == film, 'nearest_neighbor_ids'].values[0]
    ...
    if results:
        ...
        for col, movie in zip(cols, results):
            with col:
                ...
                st.markdown(f"⭐ Note : {movie['averageRating']} (Nombre de votes : {movie['numVotes']})")
